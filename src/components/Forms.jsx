...
const Forms = ({submitForm, api, formTitle, formTitleAlignment, formWidth, inputs, editors, buttonInfo, handleEditorFunction, exitForm, watch }) => {
  return (
        ...
        <div className="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl w-full">
            <div className="max-w-lg mx-auto">
              ...
              <div className='flex items-center justify-between'>
               { formTitle && <h1 className={`${formTitleAlignment} text-2xl font-semibold py-2`}>{formTitle}</h1>}
                  {exitForm && <div onClick={exitForm}><IoClose size={25} className='cursor-pointer'/></div>}
              </div>
    ...
    <div key={index}>
        <label className='text-gray-600 text-sm'>{editor.heading}:
        {editor.errorMessage  && <p className='text-xs text-red-600'>{editor.errorMessage}</p>}
      </label>
      <Editor
...
/>
        ...
        {/* <Editor handle_html={(html) => handleEditorFunction(editor.field, html)} /> */}
      </div>
    )}    
